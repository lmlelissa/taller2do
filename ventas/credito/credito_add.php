                            ...
                            <div class="box box-success">
                                ...
                                <form action="credito_control.php" method="POST" accept-charset="UTF-8">
                                    <div class="box-body">
                                        ...
                                        <input type="hidden" name="vnc_fecha_recibido" value="<?= $fechaActual; ?>"/>
                                        <input type="hidden" name="vnc_estado" value="PENDIENTE"/>
                                        ...
                                        <?php
                                        $tim = consultas::get_datos("SELECT * FROM ref_timbrados WHERE id_tipo_doc='NOTA DE CREDITO' AND estado='ACTIVO'");
                                        if (!empty($tim)) {
                                            ?>
                                            <div class = "form-group">
                                                <label >N°Timbrado</label>
                                                <?php $tim = consultas::get_datos("SELECT * FROM ref_timbrados WHERE id_tipo_doc='NOTA DE CREDITO' AND estado='ACTIVO' AND id_caja IN (SELECT id_caja FROM ref_cajas WHERE caj_estado='ABIERTA' AND id_sucursal=" . $_SESSION['id_sucursal'] . ")"); ?>
                                                <input class="form-control" type="text" name="vid_tim" value="<?= $tim[0]['id_tim']; ?>" readonly=""/> 
                                            </div>
                                            <div class="form-group">
                                                <label>Nro Documento</label>
                                                <?php $nrofact = consultas::get_datos("SELECT * FROM ref_timbrados WHERE estado='ACTIVO' AND id_tipo_doc='NOTA DE CREDITO' AND id_caja IN (SELECT id_caja FROM ref_cajas WHERE caj_estado='ABIERTA' AND id_sucursal=" . $_SESSION['id_sucursal'] . ")"); ?>
                                                <input class = "form-control" type = "text" name = "vnc_comprobante" readonly = "" value = "<?= '00' . $_SESSION['id_sucursal'] . '-00' . $nrofact[0]['id_caja'] . '-00' . ($nrofact[0]['numero_actual'] + 1); ?>">
                                            </div>
                                            <?php
                                        } else {
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2" name="vid_motivo" required="" >
                                                <?php $moti = consultas::get_datos("SELECT * FROM ref_motivo_nota WHERE id_motivo NOT IN(2,3)"); ?>
                                                <?php if (!empty($moti)) { ?>
